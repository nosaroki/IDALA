...
Deno.serve(async (req) => {
  ...
  try {
    ...
    // Comparaison en heure BRUTE de Paris des deux côtés : on ne convertit PAS ici.
    ...
    // ---- Convertir le nouveau créneau (Paris) en instant UTC ----
    // Utilisé pour tout ce qui devient un vrai instant : réservation SuperSaaS,
    // room Daily, écriture en base, mail. La comparaison ci-dessus restait en brut.
    const newStartUTC = parisToUTC(new_start);
    ...
    // ---- Supprimer l'ANCIENNE room Daily ----
    ...
    if (scheduleId) {
      try {
        const startDate = new Date(newStartUTC);
    ...
    // ---- Créer la NOUVELLE room Daily si visio ----
    ...
    if (session.mode_seance === 'visio') {
      try {
        const scheduledAt = new Date(newStartUTC);
    ...
    // ---- Mettre à jour la SESSION avec le nouvel horaire (en UTC) ----
    await supabase
      .from('sessions')
      .update({
        scheduled_at: newStartUTC,
...
async function sendModificationEmails(p: {
  ...
  const modifyUrl = `${SITE_URL}/#/modifier/${p.cancelToken}`;
  const cancelUrl = `${SITE_URL}/#/annulation/${p.cancelToken}`;

  const clientActionsFr = p.cancelToken ? `
  ...
  const clientActionsEn = p.cancelToken ? `
    <div style="margin-top:24px;padding-top:20px;border-top:1px solid #E4D8F5;">
  ...
  const idalaHtml = wrap('Rendez-vous modifié', `
    <p style="font-size:15px;color:#413459;line-height:1.7;margin:0 0 8px;text-align:center;">
      Un client a déplacé son rendez-vous.
    </p>
    <table style="width:100%;border-collapse:collapse;margin:8px 0 24px;">
      ${row('Séance', p.pratiqueNom)}
      ${row('Praticien', p.praticienPrenom)}
      ${row('Client', p.clientName)}
  ...
  await send(
    ...
    `Rendez-vous modifié : ${p.pratiqueNom} - ${p.clientName}`,
    idalaHtml
  );
}
