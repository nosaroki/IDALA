  ...
  try {
    ...
    try {
      ...
      const accessBlock = mode === 'visio' && roomUrl
        ? (clientLang === 'en'
            ? `<p style="font-size:13px;color:#413459;line-height:1.7;margin:0 0 16px;">Join your session here:<br/><a href="${roomUrl}" style="color:#9B6EBF;">${roomUrl}</a></p>`
            : `<p style="font-size:13px;color:#413459;line-height:1.7;margin:0 0 16px;">Rejoignez votre échange ici :<br/><a href="${roomUrl}" style="color:#9B6EBF;">${roomUrl}</a></p>`)
        : (clientLang === 'en'
