...
export default function OnboardingPaymentReturn() {
  ...
  useEffect(() => {
    ...
    // On interroge Stripe en direct via la fonction edge check-stripe-status,
    // au lieu de lire stripe_accounts depuis le front (lecture bloquée par la RLS
    // pour un utilisateur anonyme). On garde un court polling au cas où Stripe
    // finalise ses vérifications juste après le retour du praticien.
    async function poll() {
      ...
      // Erreur réseau ou serveur : on retente quelques fois avant d'abandonner
      if (error || !data) {
      ...
      // Token inconnu : lien invalide
      if (data.found === false) {
        setStatus('error')
        return
      }

      if (data.prenom) setPraticien({ prenom: data.prenom })
  ...
  async function handleRetry() {
    ...
    // On retrouve l'id du praticien via son token pour relancer un lien Stripe frais
    ...
    if (pErr || !prat) { setRetrying(false); return }
