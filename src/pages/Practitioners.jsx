...
// Découpe la liste en lignes quinconce alternées (4-3-4-3...) selon le nombre réel de pratiques
function buildQuinconceRows(total) {
  const rows = [];
  let start = 0;
  let big = true;
  while (start < total) {
    const count = Math.min(big ? 4 : 3, total - start);
...
function PractCard({ p, lang, colorIndex = 0 }) {

  const navigate = useNavigate();
  const chakraKeys = ['c1', 'c2', 'c3', 'c4', 'c5', 'c6', 'c7'];
  const chakraKey = chakraKeys[colorIndex % 7];

  return (
    <div className="pcard-wrap" style={{ '--chakra-color': CHAKRA_COLORS[chakraKey] }}>
      ...
      <div className="pcard-face pcard-back">
        ...
        <button
          className={`pcard-back__btn btn--chakra-pill btn--chakra-pill--${chakraKey}`}
        ...
        <section className="pract-grid-section">
          ...
          {activeFilter === 'all' ? (
            <div className="pract-grid-quinconce">
              {buildQuinconceRows(PRACTS.length).map((row, i) => (
                  ...
                  {PRACTS.slice(row.start, row.start + row.count).map((p, j) => (
                    <div key={p.titleEn} className="pcard-anim">
                      <PractCard p={p} lang={lang} colorIndex={row.start + j} />
                    </div>
                  ))}
                </div>
              ))}
            ...
            <div className={`pract-grid pract-grid--${filtered.length <= 4 ? 'sm' : 'md'}`}>
              ...
              {filtered.map((p, i) => (
                <div key={p.titleEn} className="pcard-anim">
                  <PractCard p={p} lang={lang} colorIndex={i} />
                </div>
              ))}
            </div>
          )}
